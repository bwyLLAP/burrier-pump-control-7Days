...
def compose_rolling_price_window(
    *,
    ...
    actual_frame = _normalise(actual, "Actual")
    one_day_frame = _normalise(one_day, "1-day forecast")
    ...
    populated = [frame for frame in (seven_day_frame, one_day_frame, actual_frame) if not frame.empty]
...
class RollingAEMOPriceService:
    ...
    def __init__(self, cache_dir: Path, *, session=None, timeout_seconds: float = 60.0) -> None:
    ...
    def fetch(self, *, now: pd.Timestamp, display_monday: pd.Timestamp) -> RollingPriceWindow:
        try:
            actual = self._actual_prices(display_monday, now)
            one_day, one_day_name = self._one_day_prices()
            pd7 = PD7DayPriceService(session=self.session).fetch_latest()
            ...
            return RollingPriceWindow(
                ...
                source_names=("AEMO Dispatch", one_day_name, pd7.source_name),
            )
