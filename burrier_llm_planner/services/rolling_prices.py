...
def compose_rolling_price_window(
    *,
    ...
    optimisation_index = pd.date_range(
        future_start, default_end, freq="30min", inclusive="left",
    ) if future_start < default_end else pd.DatetimeIndex([], tz=ZONE_NAME)
    ...
    combined = combined[combined["DateTime"] >= min(monday, future_start)].sort_values("DateTime")
    combined = cap_weekly_prices(combined.reset_index(drop=True))

    available_end = (
        combined["DateTime"].max() + pd.Timedelta(minutes=30)
        if not combined.empty else default_end
    )
    display_end = max(default_end, available_end)
    ...
    required_index = pd.date_range(
        min(monday, future_start), default_end, freq="30min", inclusive="left",
    )
    required_missing = combined.reindex(required_index)["Price"].isna()
    if required_missing.any():
        examples = ", ".join(
            stamp.strftime("%a %d %b %H:%M") for stamp in required_index[required_missing][:5]
        )
        raise RollingPriceError(
            f"The displayed price window has {int(required_missing.sum())} missing half-hour "
...
class RollingAEMOPriceService:
    ...
    def fetch(
        ...
        original_fallback: pd.DataFrame | None = None,
    ) -> RollingPriceWindow:
        try:
            actual = None
            one_day = None
            ...
            if self.kwatch_api_key:
                ...
                try:
                    ...
                    source_names.append(
                        "KWatch dispatch30mins + live 5min bridge"
                        if not actual.empty else "KWatch actual unavailable · using Original"
                    )
                except KWatchPriceError as exc:
                    if strict_kwatch:
                        actual = pd.DataFrame(columns=["DateTime", "Price"])
                        source_names.append("KWatch actual unavailable · using Original")
                    else:
                        actual = None
                except Exception as exc:
                    if strict_kwatch:
                ...
                try:
                    ...
                    source_names.append(
                        "KWatch predispatch30mins"
                        if not one_day.empty else "KWatch predispatch unavailable · using longer forecast"
                    )
                except KWatchPriceError as exc:
                    if strict_kwatch:
                        one_day = pd.DataFrame(columns=["DateTime", "Price"])
                        source_names.append("KWatch predispatch unavailable · using longer forecast")
                    else:
                        one_day = None
                except Exception as exc:
                    if strict_kwatch:
            ...
            if original_fallback is not None and not original_fallback.empty:
                monday_fallback = original_fallback.loc[:, ["DateTime", "Price"]].copy()
                monday_source = "saved Original weekly plan"
            else:
                try:
                    ...
                    # source. Final current-week validation still prevents an
                    # incomplete optimisation input.
                    pass
