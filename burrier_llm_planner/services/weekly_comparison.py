...
def compare_daily_windows(
    *,
    ...
    if not (len(timestamps) == len(prices) == len(comparison_states)):
    ...
    target_volume_ml = float(comparison_states.sum() * interval_volume_ml)
    if comparison_states.sum() == 0:
        ...
        result = run_aemo_weekly_optimization(AEMOWeeklyInputs(
            ...
            weekly_target_ml=target_volume_ml,
            flow_lps=flow_lps,
            max_power_kw=power_kw,
            standby_power_kw=0.0,
            minimum_continuous_run_hours=min(
                minimum_continuous_run_hours, comparison_hours,
            ),
