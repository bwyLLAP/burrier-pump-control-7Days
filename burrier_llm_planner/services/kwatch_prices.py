...
class KWatchPriceService:
    ...
    def fetch_actual(self, *, start, now) -> pd.DataFrame:
        ...
        expected = pd.date_range(start, future_start, freq="30min", inclusive="left")
        combined = combined.set_index("DateTime").reindex(expected)
        missing = combined["Price"].isna()
        if missing.any():
            raise KWatchPriceError(
                f"KWatch actual data is missing {int(missing.sum())} half-hour prices."
            )
        combined.index.name = "DateTime"
        return combined.reset_index().loc[:, ["DateTime", "Price"]]
    ...
    def fetch_predispatch(self, *, now) -> pd.DataFrame:
        ...
        if forecast.empty:
            raise KWatchPriceError("KWatch predispatch returned no future half-hours.")
        expected = pd.date_range(
            future_start, forecast["DateTime"].max(), freq="30min",
        )
        forecast = forecast.set_index("DateTime").reindex(expected)
        missing = forecast["Price"].isna()
        if missing.any():
            raise KWatchPriceError(
                f"KWatch predispatch is missing {int(missing.sum())} half-hour prices."
            )
        forecast.index.name = "DateTime"
        return forecast.reset_index().loc[:, ["DateTime", "Price"]]
