...
def _daily_comparisons(
    *, frozen: FrozenSchedule, display_frame: pd.DataFrame,
    ...
    if now.date() < frozen.monday or now.date() > week_end:
        ...
        last_day = min(week_end, tomorrow)
    if (
        tomorrow > week_end
    ...
    for offset in range((last_day - frozen.monday).days + 1):
        ...
        if not frozen_day.empty:
            ...
            comparison_label = "Monday's schedule"
        else:
        ...
        if day in replaced_dates and not frozen_day.empty:
            ...
            comparison_label = "Monday's schedule · Override"
            cost_override = (
...
def render_weekly_workspace():
    ...
    metrics = [
        ("Future 7-day volume", f"{future_volume:.2f} ML"),
        ("Future run hours", f"{future_hours:.1f} h"),
        ("Actual − replaced baseline", f"{correction.actual_minus_planned_ml:+.2f} ML"),
    ...
    with chart_column:
        with st.container(border=True):
            ...
            st.caption(
                ...
                "the additional forecast days. Pale green is the frozen Monday's schedule; stronger green "
                "is the adjusted rolling plan; blue shows applied Overrides."
            )
    with override_column:
        with st.container(border=True):
            ...
            if st.button(
                "Restore Monday's schedule", type="secondary", width="stretch",
                disabled=not applied_for_day or frozen is None,
                key="weekly_restore_monday_schedule",
            ):
                restored_dates = set(saved_replaced_dates)
                restored_dates.discard(selected_override_day)
                st.session_state.weekly_actual_runs = _replace_day_runs(
                    saved_runs, selected_override_day, [],
                )
                st.session_state.weekly_actuals_applied = bool(restored_dates)
                st.session_state.weekly_actual_replaced_dates = sorted(restored_dates)
                st.session_state.pop("weekly_result", None)
                st.session_state.pop("weekly_daily_comparisons", None)
                st.session_state.weekly_actual_editor_version = (
                    st.session_state.get("weekly_actual_editor_version", 0) + 1
