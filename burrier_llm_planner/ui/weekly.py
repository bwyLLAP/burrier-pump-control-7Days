...
from burrier_llm_planner.optimization.aemo_weekly import AEMOWeeklyInputs, run_aemo_weekly_optimization
...
from burrier_llm_planner.services.rolling_prices import (
    RollingAEMOPriceService, RollingPriceWindow, calendar_week_prices,
    calculate_volume_correction,
)
from burrier_llm_planner.services.weekly_actuals import (
    ActualPumpRun, ActualRunSummary, planned_volume_for_dates,
...
def build_weekly_figure(
    ...
    sources = (
        [("Actual", "#172033"), ("1-day forecast", "#2563A6"), ("7-day forecast", "#7C3AED")]
        if "PriceSource" in working.columns else [("Forecast price", "#2563A6")]
    )
    for source, color in sources:
        ...
        if subset.empty:
            continue
        figure.add_trace(go.Scatter(
            x=subset["DisplayTime"], y=subset["Price"], name=source, mode="lines",
            line=dict(color=color, width=2),
            hovertemplate="%{x|%a %d %b %H:%M}<br>AUD %{y:.2f}/MWh<extra></extra>",
        ))
        ...
        for first, stop in zip(changes[:-1], changes[1:]):
            if states[first]:
                ...
                figure.add_vrect(
                    ...
                    fillcolor=fillcolor, line_color=linecolor, line_width=1, layer="below",
                )
    ...
    for label, color in [
        ("No pumping · Mon–Fri 16:00–20:00", "#DC4141"),
        ("Calendar-week baseline plan", "#B0DABB"),
        ("Adjusted rolling 7-day plan", "#89CF9B"),
        ("Override", "#477EAA"),
    ]:
        figure.add_trace(go.Scatter(
    ...
    figure.update_layout(
        ...
        margin=dict(l=15, r=15, t=20, b=20), hovermode="x unified",
...
def _daily_comparisons(
    *, frozen: FrozenSchedule, display_frame: pd.DataFrame,
    ...
    for offset in range(7):
        ...
        if current_day.empty or frozen_day.empty:
            comparisons.append(None)
            continue
        aligned_frozen = frozen_day.reindex(current_day.index)
        if aligned_frozen["PumpOn"].isna().any():
            comparisons.append(None)
            continue
        if day in replaced_dates:
            day_runs = _runs_touching_day(saved_runs, day)
            comparison_states = states_for_runs(current_day.index, day_runs)
            comparison_label = "Executed actual"
            cost_override = (
            ...
            comparison_states = aligned_frozen["PumpOn"].to_numpy(float)
            comparison_label = "Released schedule (assumed)"
            cost_override = None
        else:
            comparison_states = aligned_frozen["PumpOn"].to_numpy(float)
            comparison_label = "Schedule released on Monday"
            cost_override = None
        sources = (
...
def _window_track(states: np.ndarray) -> str:
    cells = "".join(
        f"<i class='window-cell {'on' if value > 0.5 else ''}'></i>" for value in states
    )
...
def _render_daily_comparison(
    ...
    saving_percent = (
        "N/A" if comparison.potential_saving_percent is None
        else f"{comparison.potential_saving_percent:+.1f}%"
    )
    decision = (
        "Review plan"
        if comparison.potential_saving_aud >= 200
        and (comparison.potential_saving_percent or 0) >= 10
        else "Keep plan"
    )
    ...
    st.markdown(
        ...
        f"<div class='day-review-head'><strong>{format_english_date(day)}{tomorrow_label}</strong>"
        f"<span>{decision}</span></div>"
        f"<div class='window-line'><em>{comparison.comparison_label}</em>{_window_track(comparison.comparison_states)}</div>"
        f"<div class='window-line'><em>Price ideal</em>{_window_track(comparison.ideal_states)}</div>"
        f"<div class='review-stats'><span>Cost <b>${comparison.comparison_cost_aud:,.0f}</b> → "
        f"<b>${comparison.ideal_cost_aud:,.0f}</b></span>"
        f"<span>Potential saving <b>${comparison.potential_saving_aud:,.0f}</b> · <b>{saving_percent}</b></span>"
        f"<span>{comparison.hours_moved:.1f} h moved</span>"
...
def render_weekly_workspace():
    ...
    with controls:
        with st.container(border=True):
            ...
            st.caption(
                ...
                "by actual-versus-baseline performance for the rolling future seven days."
            )
    ...
    if st.session_state.get("weekly_auto_fetch_token") != fetch_token:
        for key in ("weekly_window", "weekly_result", "weekly_result_fingerprint", "weekly_load_error"):
            st.session_state.pop(key, None)
        if os.getenv("BURRIER_DISABLE_AUTO_FETCH", "").lower() not in {"1", "true", "yes"}:
            try:
                with st.spinner("Building the actual and forecast price window…"):
                ...
                st.session_state.weekly_auto_fetch_token = fetch_token
    ...
    with source_panel:
        with st.container(border=True):
            ...
            if window is not None:
                if _is_rolling_window(window):
                    st.success("Ready · actual + 1-day + 7-day AEMO prices")
                ...
                st.warning("Automatic retrieval is unavailable. Upload a saved PD7Day snapshot to use the legacy fallback.")
                st.caption(st.session_state.weekly_load_error)
            else:
                st.info("Retrieving actual, one-day and seven-day prices…")
            with st.expander("Use a saved forecast file"):
                upload = st.file_uploader(
                    "Saved PD7Day ZIP or snapshot CSV", type=["zip", "csv"], key="weekly_upload",
                )
                st.caption("Use this fallback when NEMWeb is blocked by the local network or firewall.")
    ...
    if frame is not None:
        ...
        try:
            ...
            lock = pd.Series(np.nan, index=range(len(frame)), dtype=float)
            start_index = 0
        except Exception as exc:
            frame_error = str(exc)
    ...
    planned_to_date = 0.0
    if frozen is not None and saved_replaced_dates:
        planned_to_date = planned_volume_for_dates(
            pd.DatetimeIndex(frozen.frame["DateTime"]),
            frozen.frame["IntervalVolumeML"].to_numpy(float),
            saved_replaced_dates,
        )
    elif reference is not None and saved_replaced_dates:
        planned_to_date = planned_volume_for_dates(
            reference.timestamps, reference.interval_volume_ml,
            saved_replaced_dates,
        )
    correction = calculate_volume_correction(
        float(target), actual_summary.achieved_volume_ml, planned_to_date,
    )
    remaining = correction.adjusted_future_target_ml
    common_params = dict(
    ...
    params = dict(
        **common_params, weekly_target_ml=remaining, prorate_partial_days=True,
    )
    ...
    if st.session_state.get("weekly_baseline_fingerprint") != baseline_current:
        ...
        reference = None
        planned_to_date = (
            planned_volume_for_dates(
                pd.DatetimeIndex(frozen.frame["DateTime"]),
                frozen.frame["IntervalVolumeML"].to_numpy(float),
                saved_replaced_dates,
            ) if frozen is not None and saved_replaced_dates else 0.0
        )
        correction = calculate_volume_correction(
            float(target), actual_summary.achieved_volume_ml, planned_to_date,
        )
    ...
    with st.container(border=True):
        ...
        with action:
            if st.button(
                ...
                st.session_state.pop("weekly_result", None)
                try:
                    with st.spinner("Building this week's baseline, then optimising the future seven days…"):
                        if frozen is None:
                            try:
                        ...
                        local_planned_to_date = 0.0
                        if frozen is not None and saved_replaced_dates:
                            local_planned_to_date = planned_volume_for_dates(
                                pd.DatetimeIndex(frozen.frame["DateTime"]),
                                frozen.frame["IntervalVolumeML"].to_numpy(float),
                                saved_replaced_dates,
                            )
                        elif saved_replaced_dates:
                            local_planned_to_date = planned_volume_for_dates(
                                baseline.timestamps, baseline.interval_volume_ml,
                                saved_replaced_dates,
                            )
                        local_correction = calculate_volume_correction(
                            float(target), actual_summary.achieved_volume_ml, local_planned_to_date,
                        )
                        future_params = dict(
                            **common_params,
                            weekly_target_ml=local_correction.adjusted_future_target_ml,
                            prorate_partial_days=True,
                        )
                        result = run_aemo_weekly_optimization(AEMOWeeklyInputs(
                            timestamps=pd.DatetimeIndex(frame["DateTime"]),
                            price_aud_per_mwh=frame["Price"].to_numpy(float),
                            fixed_pump_on=lock.to_numpy(float),
                            optimisation_start_index=start_index, **future_params,
                        ))
                    ...
                    reference = baseline
                    planned_to_date = local_planned_to_date
                    correction = local_correction
                    remaining = local_correction.adjusted_future_target_ml
                except Exception as exc:
                    st.error(f"No schedule generated: {exc} Check the remaining target and available hours.")
        with balance:
            st.markdown(
                f"<div class='balance-card'><span>Actual − replaced baseline</span><strong>{correction.actual_minus_planned_ml:+.2f} ML</strong>"
                f"<span>Adjusted next-7-day target</span><strong>{remaining:.2f} ML</strong></div>",
                unsafe_allow_html=True,
            )
    ...
    actual_hours = float(actual_summary.rows["Duration (h)"].sum()) if not actual_summary.rows.empty else 0.0
    metrics = [
        ("Future 7-day volume", f"{future_volume:.2f} ML", f"{remaining:.2f} ML next-7-day target"),
        ("Future run hours", f"{future_hours:.1f} h", f"{actual_hours:.1f} h actual this week"),
        ("Actual − replaced baseline", f"{correction.actual_minus_planned_ml:+.2f} ML", f"{actual_summary.achieved_volume_ml:.2f} actual · {planned_to_date:.2f} replaced baseline"),
        ("Data status", "Plan ready" if result is not None else "Prices ready", f"{len(frame)} future half-hours"),
    ]
        ...
        with st.container(border=True):
            ...
            st.plotly_chart(
                build_weekly_figure(
                    ...
                    baseline_result=reference, replaced_dates=saved_replaced_dates,
                ),
                width="stretch",
            ...
            st.caption(
                ...
                "the additional forecast days. Pale green is the current calendar-week baseline; stronger "
                "green is the adjusted rolling plan; blue shows applied Overrides."
            )
    with override_column:
        with st.container(border=True):
            st.markdown("<div class='panel-title'>Confirm actual pumping</div>", unsafe_allow_html=True)
            if frozen is None:
                st.info("Optimise once to create Schedule released on Monday.")
            ...
            st.caption(
                ...
                else "Draft from Schedule released on Monday"
            )
    ...
    with st.container(border=True):
        ...
        if frozen is None:
            ...
            st.info("Schedule released on Monday is not available yet. Optimise to create it.")
            if detail:
                st.caption(detail)
        else:
            st.caption(
                f"Schedule released on Monday · forecast issued "
            ...
            for offset, comparison in enumerate(daily_comparisons):
                comparison_day = monday + timedelta(days=offset)
    ...
    with st.container(border=True):
        ...
        if result is not None:
            st.caption(
                f"Base next-7-day target {target:.2f} ML · actual minus replaced baseline {correction.actual_minus_planned_ml:+.2f} ML · "
                f"adjusted target {remaining:.2f} ML · future scheduled {future_volume:.2f} ML."
            )
