    ...
    with source_panel:
        with st.container(border=True):
            ...
            if window is not None:
                ...
                st.warning("Price retrieval failed. Check the selected source, API key and network access.")
    ...
    with override_column:
        with st.container(border=True):
            ...
            if st.button(
                ...
                disabled=(
                    frame is None or lock is None
                    or not st.session_state.get("weekly_reoptimisation_pending", False)
                ),
                key="weekly_reoptimise_after_override",
            ):
                try:
                    with st.spinner("Reoptimising the future seven-day plan…"):
                        revised = run_aemo_weekly_optimization(AEMOWeeklyInputs(
                            timestamps=pd.DatetimeIndex(frame["DateTime"]),
                            price_aud_per_mwh=frame["Price"].to_numpy(float),
                            fixed_pump_on=lock.to_numpy(float),
                            optimisation_start_index=start_index,
                            **params,
                        ))
                    ...
                    st.error(f"Future plan not reoptimised: {exc}")
