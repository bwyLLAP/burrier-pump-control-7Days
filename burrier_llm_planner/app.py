...
def render_strategy_workspace(demo_mode: bool) -> None:
    render_brand_header(STRATEGY_WEEKLY)
...
st.markdown(
    ...
    html { font-size:clamp(14px, calc(11px + .24vw), 17px); }
    ::selection { background:#cce5e6; color:var(--ink); }
    .stApp { background:#fbfcfc; color:var(--ink); }
    .block-container { width:min(96vw, 1760px); max-width:none; padding:clamp(2.8rem, 4vw, 3.75rem) clamp(1rem, 2.2vw, 2.25rem) 5rem; }
    ...
    .subpanel-title { color:var(--ink); font-size:.82rem; line-height:1.35; font-weight:680; margin:1rem 0 .45rem; }
    ...
    .day-review-head { margin-bottom:.38rem; }
    ...
    .window-cell { display:block; min-width:1px; background:#e3eaec; cursor:help; }
    ...
    .override-ledger { display:grid; gap:.48rem; margin-bottom:.7rem; }
    .override-ledger-row { display:grid; grid-template-columns:minmax(0,1fr); gap:.08rem; padding:.52rem .62rem; background:var(--surface); border:1px solid var(--line); border-radius:7px; font-variant-numeric:tabular-nums; }
    .override-ledger-row strong { color:var(--ink); font-size:.77rem; }
    .override-ledger-row span { color:var(--ink); font-size:.76rem; overflow-wrap:anywhere; }
    .override-ledger-row small, .override-empty { color:var(--muted); font-size:.7rem; line-height:1.4; }
    .override-empty { margin:.1rem 0 .65rem; }
