...
class AEMOWeeklyInputs:
    ...
    minimum_daily_run_steps_by_date: dict[date, int] | None = None
...
def run_aemo_weekly_optimization(inputs: AEMOWeeklyInputs) -> AEMOWeeklyResult:
    ...
    explicit_daily_steps = inputs.minimum_daily_run_steps_by_date or {}
    unknown_dates = set(explicit_daily_steps) - set(day_indices)
    if unknown_dates:
        raise ValueError("Daily pumping floors contain dates outside the optimisation horizon.")
    for day, required_steps in explicit_daily_steps.items():
        if (
            isinstance(required_steps, bool)
            or not isinstance(required_steps, (int, np.integer))
            or required_steps < 0
            or required_steps > len(day_indices[day])
        ):
            raise ValueError("Daily pumping floors must be non-negative whole half-hour steps within each day.")
    restricted = {
    ...
    for day, indices in day_indices.items():
        ...
        required_daily = max(required_daily, int(explicit_daily_steps.get(day, 0)))
...
def run_aemo_two_stage_rolling_optimization(
    allocation_inputs: AEMOWeeklyInputs,
    operating_price_aud_per_mwh: np.ndarray,
) -> tuple[AEMOWeeklyResult, AEMOWeeklyResult]:
    """Allocate volume by 7-day prices, then optimise windows without moving it between days."""

    allocation = run_aemo_weekly_optimization(allocation_inputs)
    dates = pd.DatetimeIndex(allocation.timestamps).date
    daily_steps = {
        day: int(np.rint(allocation.pump_on[np.asarray(dates) == day]).sum())
        for day in pd.unique(dates)
    }
    final_inputs = replace(
        allocation_inputs,
        price_aud_per_mwh=np.asarray(operating_price_aud_per_mwh, dtype=float),
        minimum_daily_run_steps_by_date=daily_steps,
    )
    final = run_aemo_weekly_optimization(final_inputs)
